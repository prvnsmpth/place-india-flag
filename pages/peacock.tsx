...
import PeacockDataUntyped from '../data/peacock.json'
...
const PeacockData: SelectedState = PeacockDataUntyped
...
export default function Peacock() {
  ...
  for (let i = 0; i < HEIGHT; i++) {
    ...
    for (let j = 0; j < WIDTH; j++) {
      ...
      const cellColor = PeacockData[i] && PeacockData[i][j] !== '' ? { background: PeacockData[i][j] } : {}
      cells[j] = (
        <td
          key={`${i}-${j}`}
          className={cellClass}
  ...
  return (
    <main className={styles.main}>
      ...
      <p className={styles.description}>
        This is how we are going to draw the peacock.
      </p>
      <p className={styles.description}>
